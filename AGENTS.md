...
- Phase 1 で LINE / CRM 等の外部サービスを接続する場合は、そのエンドポイントを **§12 の allowlist に追記してからコードに反映する**。allowlist 未追記のまま通信実装を進めることは禁止
...
> Adapter / SDK / ラッパークラスを新規導入する際は、**§12 のセキュリティチェック手順を必ず実施すること**。  
...
10. **既存コードのimportを無条件に踏襲しない**：既存ファイルで使われているパッケージであっても、新しいファイルで `import` する前に §12 のセキュリティチェックを実施する。`requirements.txt` に記載のないパッケージは使用禁止。**新規追加コードの差分確認だけで安全と判断してはならない。追加コードが参照する依存パッケージ本体・SDKラッパーの実装まで遡って確認すること**
...
| **差分確認だけで安全と判断しない** | 新規追加コードの差分確認だけでは不十分。追加コードが参照する既存モジュール・依存パッケージ本体・SDKラッパーの実装まで確認すること |
...
本リポジトリで許可する通信先ドメインを以下に限定する。追加する場合はこのリストを更新したうえでレビューを受けること。

```
# 許可済みドメイン
*.googleapis.com          # Vertex AI / Cloud Run / GCP APIs
*.google.com              # Google Cloud SDK
pypi.org                  # pip パッケージ取得
files.pythonhosted.org    # pip パッケージ取得
```

> **Note**  
> Phase 1 で LINE / CRM 等の外部サービスを接続する際は、そのエンドポイントをここに追記してからコードに反映すること。
...
- [ ] 外部HTTPリクエストの送信先がすべて allowlist 内のドメインか
