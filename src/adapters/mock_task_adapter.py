"""タスク作成向けの Mock Adapter を提供する。

概要:
    create_followup_task primitive を模したインメモリ adapter を実装する。
入出力:
...
制約:
    ...
    - 監査情報はメモリ上にのみ保持する
Note:
    - Phase 1 の mock 実装であり、本番タスク管理基盤への接続は行わない
...
class MockTaskAdapter:
    ...
    def execute(self, primitive_name: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        execution_id = f"task-exec-{self._execution_count:03d}"
        dry_run = bool(payload.get("dry_run", False))

        if not dry_run:
            self._tasks.append({"primitive": primitive_name, "payload": payload.copy()})
