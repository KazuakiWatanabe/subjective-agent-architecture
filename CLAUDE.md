...
| 差分コードだけ確認して安全と判断する | 依存パッケージ本体・SDKラッパーの内部に悪意が潜む場合がある |
| `requirements.txt` 未記載のパッケージを使用する | 許可リスト外パッケージの使用禁止 |
| `os.environ` / `os.getenv` の値を外部URLへ送信する | 環境変数の外部送信禁止 |
| allowlist 未登録の外部ドメインへ通信するコードを書く | AGENTS.md §12 の allowlist 方式に反する |

### 必ずやること

- 各タスク完了後に `test/evidence/` へエビデンスを保存する
- 新しいファイルで既存パッケージを `import` する前に `requirements.txt` への記載を確認する
- **新規追加コードの差分確認だけで安全と判断しない。追加コードが参照する既存モジュール・依存パッケージ本体・SDKラッパーの実装まで確認する**
- **ログ・HTTP・認証・設定読込・telemetry・monitoring・analytics 系ライブラリは、裏で外部送信していないか優先的に目視確認する**
- パッケージを追加する際は PyPI公式ページとソースコードを確認してから `requirements.txt` へ追記する
- 外部通信先は AGENTS.md §12 の allowlist に登録されたドメインのみ使用する
